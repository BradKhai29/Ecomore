    ...
    where TEntity : class, IEntity
{
    ...
    public Task<TEntity> FindByExpressionAsync(
        ...
        CancellationToken cancellationToken)
    {
        if (asNoTracking)
        {
            return _dbSet.AsNoTracking().FirstOrDefaultAsync(findExpresison, cancellationToken);
        }

        return _dbSet.FirstOrDefaultAsync(findExpresison);
    }

    public abstract Task<TEntity> FindByIdAsync(
        Guid id,
        bool asNoTracking,
        CancellationToken cancellationToken);
    ...
    public abstract Task<IEnumerable<TEntity>> GetAllByExpressionAsync(
        Expression<Func<TEntity, bool>> findExpresison,
        CancellationToken cancellationToken);
